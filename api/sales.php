...
function createSale(array $input,array $user):never {
    ...
    try{
        ...
        $pdo->commit();
        upsertPatientFromSale($pdo, $customerName, $customerPhone);

        // Best-effort receipt notification - a completed sale must never
        // fail or roll back because a text message didn't go out. Opt-in
        // per sale via send_receipt_sms (defaults on when a phone number was
        // given, since that's what the checkout UI defaults its checkbox to).
        $wantsSms = $customerPhone !== null && ($input['send_receipt_sms'] ?? true);
        if ($wantsSms) {
            try {
                $channel = strtolower((string)($input['receipt_channel'] ?? 'sms'));
                sendSms($customerPhone, buildReceiptSmsText($invoiceNo, $finalAmount), $channel === 'whatsapp');
            } catch (Throwable $e) {
                error_log('Receipt SMS failed for sale ' . $saleId . ': ' . $e->getMessage());
            }
        }

        logActivity('sale_completed','sale',$saleId,$user,['invoice_no'=>$invoiceNo,'final_amount'=>$finalAmount]);jsonResponse(['success'=>true,'sale'=>['id'=>$saleId,'invoice_no'=>$invoiceNo,'subtotal'=>round($subtotal,2),'discount_amount'=>round($discountTotal,2),'tax_amount'=>round($taxTotal,2),'final_amount'=>$finalAmount,'payment_mode'=>$paymentMode]],201);
...
function handleSales(array $segments,array $user):never{if(($segments[1]??'')===''&&requestMethod()==='POST')createSale(requestBody(),$user);if(($segments[1]??'')==='list'&&requestMethod()==='GET'){$pdo=db();$limit=min(max((int)($_GET['limit']??100),1),500);$stmt=$pdo->query('SELECT id,invoice_no,customer_name,customer_phone,payment_mode,total_amount,discount_amount,tax_amount,return_amount,final_amount,status,billed_by,created_at FROM sales_log ORDER BY created_at DESC LIMIT '.$limit);$sales=$stmt->fetchAll();if($sales){$ids=array_column($sales,'id');$ph=implode(',',array_fill(0,count($ids),'?'));$itemsStmt=$pdo->prepare("SELECT si.*, m.name AS medicine_name FROM sale_items si INNER JOIN medicines m ON m.id=si.medicine_id WHERE si.sale_id IN ({$ph})");$itemsStmt->execute($ids);$bySale=[];foreach($itemsStmt->fetchAll() as $item){$bySale[$item['sale_id']][]=$item;}foreach($sales as &$sale){$sale['items']=$bySale[$sale['id']]??[];}unset($sale);}jsonResponse(['success'=>true,'sales'=>$sales]);}jsonResponse(['success'=>false,'message'=>'Unsupported sales operation.'],405);}
