...
function lockReturnBatch(PDO $pdo, int $medicineId, string $batchNumber): array {
    ...
    if ($batchNumber === '') throw new InvalidArgumentException('batch_number is required for batch-level stock operations.');
...
function customerReturn(array $input, array $user): never {
    ...
    if ($invoice === '' || $medicineId <= 0 || $quantity <= 0) {
        jsonResponse(['success'=>false,'message'=>'invoice_no, medicine_id and positive quantity are required.'],422);
    }
    ...
    try {
        ...
        $stmt=$pdo->prepare('SELECT batch_number, COALESCE(SUM(quantity),0) AS sold_quantity, MAX(unit_price) AS unit_price FROM sale_items WHERE sale_id=? AND medicine_id=? GROUP BY batch_number');
        $stmt->execute([(int)$sale['id'],$medicineId]);
        $soldRows=$stmt->fetchAll();
        if (!$soldRows) throw new InvalidArgumentException('This medicine is not present on the invoice.');

        if ($requestedBatch === '') {
            if (count($soldRows) !== 1) throw new InvalidArgumentException('This invoice contains multiple batches for the medicine. batch_number is required for the return.');
            $requestedBatch = trim((string)($soldRows[0]['batch_number'] ?? ''));
        }

        $sold = null;
        foreach ($soldRows as $row) {
            if ((string)($row['batch_number'] ?? '') === $requestedBatch) { $sold = $row; break; }
        }
        if (!$sold) throw new InvalidArgumentException('The selected batch was not sold on this invoice.');
        ...
        $remainingEligible=(int)$sold['sold_quantity']-$already;
        if ($quantity > $remainingEligible) throw new InvalidArgumentException('Return quantity exceeds the quantity still eligible for this batch.');
        ...
        $batchResult=adjustReturnBatch($pdo,(int)$batch['id'],$quantity,$base,true);
        ...
        jsonResponse(['success'=>true,'return_no'=>$returnNo,'refund_amount'=>$refund,'batch_number'=>$requestedBatch,'batch_quantity'=>$batchResult[0],'stock_quantity'=>$newQty,'unit_label'=>$unit],201);
...
function dealerReturn(array $input, array $user): never {
    ...
    try{
        ...
        $unit=strtolower((string)$medicine['unit_label']);
        $base=returnBaseQty($medicine,$quantity);
        $batch=lockReturnBatch($pdo,$medicineId,$batchNumber);
        if((int)$batch['quantity']<$quantity || (int)$batch['base_quantity']<$base)throw new InvalidArgumentException('Cannot return more stock from the selected batch than is available.');
        $dealerName=null;
        if($dealerId!==null){
            $stmt=$pdo->prepare('SELECT name FROM dealers WHERE id=? LIMIT 1 FOR UPDATE');
            $stmt->execute([$dealerId]);
            $dealerName=$stmt->fetchColumn();
            if($dealerName===false)throw new InvalidArgumentException('Dealer not found.');
        }
        $returnNo=returnNumber('DR');
        $returnId=random_int(1000000000,9999999999);
        ...
        $batchResult=adjustReturnBatch($pdo,(int)$batch['id'],$quantity,$base,false);
        [$newQty,$newBase]=recalculateReturnMedicineStock($pdo,$medicineId);
        $stmt=$pdo->prepare('INSERT INTO stock_movements (medicine_id,batch_number,movement_type,quantity,unit_label,quantity_in_base_units,reference_type,reference_id,reason,performed_by) VALUES (?,?,?,?,?,?,?,?,?,?)');
        $stmt->execute([$medicineId,$batchNumber,'dealer_return',$quantity,$unit,$base,'dealer_return',$returnId,$reason,$user['id']??null]);
        $pdo->commit();
        logActivity('dealer_return_completed','dealer_return',$returnId,$user,['return_no'=>$returnNo,'medicine_id'=>$medicineId,'batch_number'=>$batchNumber,'quantity'=>$quantity]);
        jsonResponse(['success'=>true,'return_no'=>$returnNo,'batch_number'=>$batchNumber,'batch_quantity'=>$batchResult[0],'stock_quantity'=>$newQty,'unit_label'=>$unit],201);
...
function disposeStock(array $input,array $user):never{
    ...
    $quantity=(int)($input['quantity']??0);
    $batchNumber=trim((string)($input['batch_number']??''));
    $reason=trim((string)($input['reason']??'Expired/damaged stock'));
    ...
    try{
        ...
        $base=returnBaseQty($medicine,$quantity);
        $batch=lockReturnBatch($pdo,$medicineId,$batchNumber);
        if((int)$batch['quantity']<$quantity || (int)$batch['base_quantity']<$base)throw new InvalidArgumentException('Cannot dispose more stock from the selected batch than is available.');
        $stmt=$pdo->prepare('INSERT INTO disposals (medicine_id,batch_number,quantity,unit_label,reason,notes,disposed_by) VALUES (?,?,?,?,?,?,?)');
        $stmt->execute([$medicineId,$batchNumber,$quantity,$unit,$reason,$notes,$user['id']??null]);
        $disposalId=(int)$pdo->lastInsertId();
        $batchResult=adjustReturnBatch($pdo,(int)$batch['id'],$quantity,$base,false);
        [$newQty,$newBase]=recalculateReturnMedicineStock($pdo,$medicineId);
        $stmt=$pdo->prepare('INSERT INTO stock_movements (medicine_id,batch_number,movement_type,quantity,unit_label,quantity_in_base_units,reference_type,reference_id,reason,performed_by) VALUES (?,?,?,?,?,?,?,?,?,?)');
        $stmt->execute([$medicineId,$batchNumber,'disposal',$quantity,$unit,$base,'disposal',$disposalId,$reason,$user['id']??null]);
        $pdo->commit();
        logActivity('stock_disposed','disposal',$disposalId,$user,['medicine_id'=>$medicineId,'batch_number'=>$batchNumber,'quantity'=>$quantity,'reason'=>$reason]);
        jsonResponse(['success'=>true,'disposal_id'=>$disposalId,'batch_number'=>$batchNumber,'batch_quantity'=>$batchResult[0],'stock_quantity'=>$newQty,'unit_label'=>$unit],201);
    }catch(InvalidArgumentException $e){if($pdo->inTransaction())$pdo->rollBack();jsonResponse(['success'=>false,'message'=>$e->getMessage()],422);}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}
}
