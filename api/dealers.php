...
function handleDealers(array $segments, array $user): never
{
    ...
    if ($action === 'purchases' && isset($segments[2]) && ctype_digit($segments[2]) && requestMethod() === 'GET') {
        $orderId = (int)$segments[2];
        $stmt = $pdo->prepare('SELECT po.*, d.name AS dealer_name FROM purchase_orders po INNER JOIN dealers d ON d.id = po.dealer_id WHERE po.id = ? LIMIT 1');
        $stmt->execute([$orderId]);
        $order = $stmt->fetch();
        if (!$order) {
            jsonResponse(['success' => false, 'message' => 'Purchase order not found.'], 404);
        }
        $itemsStmt = $pdo->prepare('SELECT pi.*, m.name AS medicine_name FROM purchase_items pi INNER JOIN medicines m ON m.id = pi.medicine_id WHERE pi.purchase_id = ? ORDER BY pi.id');
        $itemsStmt->execute([$orderId]);
        $order['items'] = $itemsStmt->fetchAll();
        jsonResponse(['success' => true, 'purchase' => $order]);
    }
...
function receivePurchase(array $input, array $user): never
{
    ...
    try {
        ...
        foreach ($items as $item) {
            ...
            $base = stockBaseQuantity($qty, $unit, (int)$item['tablets_per_strip']);

            // medicine_batches is the real source of truth for sellable stock
            // (FEFO sales pull from it directly) - write through it here
            // rather than stomping the medicines row's cached totals, or this
            // stock would be invisible at the till.
            $batchStmt = $pdo->prepare('SELECT * FROM medicine_batches WHERE medicine_id = ? AND batch_number = ? LIMIT 1 FOR UPDATE');
            $batchStmt->execute([$medicine['id'], $item['batch_number']]);
            $batch = $batchStmt->fetch();
            $mrp = $item['mrp'] !== null ? (float)$item['mrp'] : (float)($medicine['strip_mrp'] ?? 0);
            $sellingPrice = $item['selling_price'] !== null ? (float)$item['selling_price'] : (float)($medicine['strip_selling_price'] ?? 0);
            if ($batch) {
                if ((string)$batch['expiry_date'] !== (string)$item['expiry_date']) {
                    throw new InvalidArgumentException('Batch ' . $item['batch_number'] . ' already exists for ' . $medicine['name'] . ' with a different expiry date.');
                }
                $pdo->prepare('UPDATE medicine_batches SET quantity = quantity + ?, base_quantity = base_quantity + ?, purchase_price = ?, mrp = ?, selling_price = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
                    ->execute([$qty, $base, $item['purchase_price'], $mrp, $sellingPrice, (int)$batch['id']]);
            } else {
                $batchId = random_int(1000000000, 9999999999);
                $pdo->prepare('INSERT INTO medicine_batches (id, medicine_id, batch_number, expiry_date, quantity, base_quantity, purchase_price, mrp, selling_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)')
                    ->execute([$batchId, $medicine['id'], $item['batch_number'], $item['expiry_date'], $qty, $base, $item['purchase_price'], $mrp, $sellingPrice]);
            }

            // Keep default pricing and dealer link current for display, without
            // touching current_stock_* here - stockRecalculate derives those
            // from the batch totals right after.
            $pdo->prepare('UPDATE medicines SET strip_purchase_price = ?, strip_mrp = COALESCE(?, strip_mrp), strip_selling_price = COALESCE(?, strip_selling_price), dealer_id = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
                ->execute([$item['purchase_price'], $mrp, $sellingPrice, $purchase['dealer_id'], $medicine['id']]);
            stockRecalculate($pdo, (int)$medicine['id']);

            recordMovement($pdo, (int)$medicine['id'], $item['batch_number'], 'purchase_received', $qty, $unit, $base, 'purchase_receipt', $receiptId, 'Goods received ' . $receiptNo, $user);
