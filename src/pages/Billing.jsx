...
export default function Billing({ isCalculatorOnly = false }) {
  const { inventory, getStockDisplay, getUnitName, getPackName, patients = [] } = useAppContext();
  ...
  const [receiptChannel, setReceiptChannel] = useState('sms'); // 'sms' or 'whatsapp'
  ...
  const handleCheckout = async () => {
    ...
    const payload = {
      ...
      send_receipt_sms: Boolean(patient.phone) && sendReceiptSms,
    ...
    <div className="billing-page animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '16px', maxWidth: '100%', overflowX: 'hidden', position: 'relative' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr) 340px', gap: '16px', width: '100%' }}>
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '16px', minWidth: 0, overflow: 'hidden' }}>
          ...
          {/* Search Input - also doubles as a barcode scanner input. A
              hardware scanner types the code then sends Enter, exactly like
              a very fast typist - so Enter checks for an exact barcode
              match before falling back to normal name/category filtering. */}
          <div style={{ position: 'relative' }}>
            <Search size={18} style={{ position: 'absolute', left: '14px', top: '12px', color: 'var(--text-secondary)' }} />
            <input
              type="text"
              className="form-input"
              placeholder="Type to filter, or scan a barcode and press Enter..."
              ...
              onKeyDown={(e) => {
                ...
                if (!scanned) return;
                const match = inventory.find(item => item.barcode && item.barcode === scanned);
                if (match) {
                  addToCart(match);
                  setSearch('');
                  return;
                }
                // Only warn on a failed scan - a barcode-looking string
                // (all digits, reasonably long) that matched nothing.
                // Ordinary text typed to filter the list shouldn't trigger
                // this on every Enter press.
                if (/^\d{6,}$/.test(scanned)) {
                  showValidation(`No medicine found with barcode "${scanned}". It may not be catalogued yet.`);
                }
              }}
              autoFocus
            />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div className="card" style={{ display: 'flex', flexDirection: 'column', flex: 1, padding: '20px' }}>
            ...
            {/* SMS / WhatsApp Receipt Opt-in - only relevant with a phone number */}
            {!isCalculatorOnly && cart.length > 0 && patient.phone && (
              <div style={{ marginBottom: '14px', borderTop: '1px solid var(--border-color)', paddingTop: '12px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px', marginTop: '8px' }}>
                    {[
                      ...
                      <button
                        key={ch.value}
                        type="button"
                        className={`btn ${receiptChannel === ch.value ? 'btn-primary' : 'btn-outline'}`}
                        style={{ padding: '5px 12px', fontSize: '11px' }}
                        onClick={() => setReceiptChannel(ch.value)}
                      >
                        {ch.label}
                      </button>
                    ))}
