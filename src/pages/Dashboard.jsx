...
export default function Dashboard() {
  const { inventory, getStockDisplay, regularPatients = [] } = useAppContext();
  ...
  const filteredSales = sales.filter(sale => {
    ...
    const matchesSearch = customerName.includes(salesSearch.toLowerCase()) || 
                          (sale.customer_phone && sale.customer_phone.includes(salesSearch));
      ...
      <div className="grid grid-cols-3">
        <div className="card">
          ...
          <div style={{ marginTop: '8px', fontSize: '13px', color: 'var(--text-secondary)' }}>
            {salesLoading ? 'Loading…' : `From ${todaySalesCount} transactions today`}
          </div>
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '16px' }}>
            <div className="flex-between">
              ...
              <button className="btn btn-outline" style={{ padding: '4px 8px', fontSize: '12px' }} onClick={() => {
                ...
                let csv = 'Invoice No,Time,Customer Name,Phone,Total Items,Total Amount (INR)\n';
                filteredSales.forEach(sale => {
                  const time = new Date(sale.created_at).toLocaleString();
                  csv += `${sale.invoice_no},"${time}","${sale.customer_name || 'Walk-in'}","${sale.customer_phone || 'N/A'}",${(sale.items || []).length},${Number(sale.final_amount).toFixed(2)}\n`;
                });
              ...
              <div style={{ padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {filteredSales.map(sale => (
                  <div key={sale.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px', backgroundColor: 'white', borderRadius: '6px', border: '1px solid var(--border-color)' }}>
                    <div>
                      <div style={{ fontWeight: '600', fontSize: '14px' }}>{sale.customer_name || 'Walk-in Customer'}</div>
