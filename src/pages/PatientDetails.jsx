...
export default function PatientDetails() {
  ...
  useEffect(() => {
    ...
    setSalesError('');
    apiGet(`patients/${patient.id}/sales`)
      .then(data => { if (!cancelled) setPatientSales(data.sales || []); })
      .catch(err => { if (!cancelled) setSalesError(err.message || 'Could not load purchase history.'); })
  ...
  return (
    <div className="patient-details">
      ...
      <div className="card">
        ...
        {salesError && (
          <div style={{ backgroundColor: '#fef2f2', border: '1px solid #fca5a5', color: '#991b1b', fontSize: '13px', fontWeight: '600', padding: '12px', borderRadius: '8px', marginBottom: '16px' }}>
            ⚠️ {salesError}
          </div>
        )}

        {salesLoading ? (
          <div style={{ textAlign: 'center', padding: '40px', color: 'var(--text-secondary)' }}>Loading purchase history…</div>
              ...
              <div key={sale.id} style={{ border: '1px solid var(--border-color)', borderRadius: '8px', padding: '16px' }}>
                <div className="flex-between" style={{ borderBottom: '1px solid var(--border-color)', paddingBottom: '12px', marginBottom: '12px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ fontWeight: '600' }}>{sale.invoice_no}</span>
                ...
                <table className="table" style={{ width: '100%', fontSize: '14px' }}>
                  ...
                  <tbody>
                    {(sale.items || []).map((item, idx) => (
                      <tr key={idx}>
                        <td style={{ padding: '8px', borderBottom: '1px solid #eee' }}>{item.medicine_name}</td>
                        <td style={{ padding: '8px', borderBottom: '1px solid #eee' }}>{item.quantity} {item.unit_label}{item.quantity === 1 ? '' : 's'}</td>
                        <td style={{ padding: '8px', borderBottom: '1px solid #eee' }}>₹{Number(item.unit_price).toFixed(2)}</td>
