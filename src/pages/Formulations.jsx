...
export default function Formulations() {
  const { inventory, formulations = [], addFormulation, deleteFormulation } = useAppContext();
  ...
  const handleSubmit = (e) => {
    ...
    addFormulation({
      ...
      unitLabel: newForm.unitLabel.trim() || 'unit',
  ...
  const handleDelete = (id, name, itemsCount) => {
    if (itemsCount > 0) {
            ...
            <form onSubmit={handleSubmit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '12px', paddingTop: '16px', borderTop: '1px solid var(--border-color)' }}>
                ...
                <button type="submit" className="btn btn-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
                  <CheckCircle size={16} /> Save Formulation Type
                </button>
              </div>
            </form>
          </div>
